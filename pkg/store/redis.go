...
func (r *RedisStore) AcquireLayerLock(ctx context.Context, layer string, afterTime time.Duration) (*RedisLock, error) {
	...
	for {
		...
		case <-ticker.C:
			...
			logger.InfoContextf(ctx, "waiting for get the distribute layer lock...")
		case <-ctx.Done():
...
func (r *RedisStore) SaveOCILayer(ctx context.Context, ociType LayerType, layer, filePath string) error {
	...
	logger.InfoContextf(ctx, "cache save oci layer '%s = %s' success", rdk, filePath)
	return nil
}
...
func (r *RedisStore) SaveStaticLayer(ctx context.Context, layer string, filePath string, printLog bool) error {
	...
	if printLog {
		logger.InfoContextf(ctx, "cache save static layer '%s = %s' success", rdk, filePath)
	}
	return nil
}
...
func (r *RedisStore) SaveTorrent(ctx context.Context, layer string, torrentBase64 string) error {
	...
	logger.InfoContextf(ctx, "cache save torrent layer '%s = (too long)' success", rdk)
	return nil
}
...
func (r *RedisStore) commonQuery(ctx context.Context, layer string, keyTypes []string) ([]*LayerLocatedInfo, error) {
	...
	for _, key := range resultKeys {
		t := strings.Split(key, "/")
		if len(t) != 3 {
			logger.WarnContextf(ctx, "redis key '%s' not normal", key)
			continue
		}
		located := t[1]
		ociType := t[2]
		...
		if err != nil {
			logger.WarnContextf(ctx, "redis key '%s' get failed: %s", key, err)
			continue
		}
